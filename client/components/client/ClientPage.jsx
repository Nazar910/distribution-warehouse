...
const ClientPage = React.createClass({
  ...
  printClients(){
    ...
    let trs = '';
    this.state.clientsQuery.forEach(function(item,i,arr){
      trs+=`<tr>`+
      `<td>${item.id}</td>`+
      `<td>${item.lastName}</td>`+
      `<td>${item.name}</td>`+
      `<td>${item.pob}</td>`+
      `<td>${item.rasch_sch}</td>`+
      `<td>${item.mfo}</td>`+
      `<td>${item.address}</td>`+
      `</tr>`
    });
    console.log(trs);
    let table=`<table border="1"><tr align="center"><th>${this.props.labels.id}</th><th>${this.props.labels.lastName}</th>`+
    `<th>${this.props.labels.name}</th><th>${this.props.labels.pob}</th><th>${this.props.labels.rasch_sch}</th><th>${this.props.labels.mfo}</th><th>${this.props.labels.address}</th>`
    +`${trs}</table>`;
  ...
  render(){
    return(
      ...
      <div><span className='Item__print' onClick={this.printClients}>{this.props.labels.print}</span></div>
      </div>
      <div className="Sort">
